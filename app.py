...
# --- PASS 1: Extraction (Reverted to Stable Static Quadrants) ---
def extract_telemetry(video_path, progress_callback):
    ...
    while cap.isOpened():
        ...
        # 3. 4-CORNER ACTIVITY (Static Quadrants)
        # This was the stable version that worked
        ...
        # YOLO for Car Box (for Fuel ROI only)
...
# --- PASS 2: Analysis V55 (Gun-Spike Logic) ---
def analyze_states_v55(df, fps):
    window = 15
    ...
    if t_up and t_down:
        ...
        def find_gun_start(sig, start_g, end_g):
            # Look for the steepest increase in activity (walking -> gun)
            mask = (times_j >= start_g) & (times_j <= end_g)
            if not np.any(mask): return start_g
            s_win = sig[mask]
            t_win = times_j[mask]
            
            # Derivative of activity
            grad = np.gradient(s_win)
            max_grad = np.max(grad)
            
            # Spike Threshold: 30% of max acceleration
            spike_thresh = max_grad * 0.3
            
            spikes = np.where(grad > spike_thresh)[0]
            if len(spikes) > 0:
                return t_win[spikes[0]]
            else:
                # Fallback to standard high threshold
        ...
        # B. REAR (Gun-Spike Logic)
        sig_ir = df_j[map_corners['Inside Rear']].values
        sig_or = df_j[map_corners['Outside Rear']].values
        
        # 1. IR Start (Sensitive)
        t_ir_start, t_ir_raw_end = get_window(sig_ir, t_up, t_ae, sens=0.2)
        
        # 2. OR Start (Gun Spike)
        # Search after IR started + reasonable time
        t_or_start = find_gun_start(sig_or, t_ir_start + 2.0, t_ae)
        
        # Find OR End
        _, t_or_end = get_window(sig_or, t_or_start, t_ae)
        
        # 3. Calculate Transition
        # Logic: IR stops exactly when transition to OR starts?
        # No, IR stops, then transition happens, then OR starts.
        
        # Force IR End to be 'logical'
        # If IR Raw End is > OR Start, it's the fueler. Cut it.
        # Transition Start = IR End.
        
        if t_ir_raw_end > t_or_start - 1.0:
            t_ir_end = t_or_start - 1.5 # Assume 1.5s transit if overlapped
        ...
        # Safety clamps
        if t_trans_end < t_trans_start: 
             t_trans_end = t_trans_start + 0.1 # Instant?
...
# --- Main ---
def main():
    st.title("🏁 Pit Stop Analyzer V55")
    st.markdown("### Gun-Spike Logic")
    st.info("Uses **Static Quadrants** (Stability) + **Derivative Logic** (Gun Spike) for accurate transition timing.")
    ...
    if uploaded_file and st.button("Start Analysis", type="primary"):
        ...
        try:
            bar = st.progress(0)
            st.write("Step 1: Extraction (Static Quadrants)...")
            df, fps, w, h = extract_telemetry(tfile.name, bar.progress)
            
            st.write("Step 2: Analysis (Gun Spike Logic)...")
            pit_t, tire_t, fuel_t, corners, df_final = analyze_states_v55(df, fps)
            ...
            with c2:
                if os.path.exists(vid_path):
                    with open(vid_path, 'rb') as f:
                        st.download_button("Download MP4", f, file_name="pitstop_v55.mp4")
        else:
